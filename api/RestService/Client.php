...
class Client
{
    ...
    public function __construct($pServerController)
    {
        ...
        {            
            $this->setUrl($_SERVER['SCRIPT_NAME']);
        }
	...
	$this->content_type = false;	
	    ...
	    $this->content_type = preg_replace('/;(.*)/', "", $this->content_type);
        }
		...
		// gets are easy...
                case 'get':
                	...
                	$data = $_GET;   
                	if(isset($this->parameters)) $data['raw_get_data'] = $this->parameters;
                	else $data['raw_get_data'] = array();
                        break;

                case 'post':    
                
                	if(count($this->parameters)) $data = $this->parameters;
                        else $data = $_POST;
